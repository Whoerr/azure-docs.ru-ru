...
Чтобы заблокировать приложение на прием трафика только от конкретной передней дверцы, необходимо настроить списки ACL IP-адресов для серверной части. Затем Ограничьте трафик серверной части до указанного значения заголовка "X-Azure-ФДИД", отправленного с помощью передней дверцы. Эти действия описаны ниже.
