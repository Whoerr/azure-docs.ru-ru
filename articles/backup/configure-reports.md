...
С помощью этой вкладки можно определить, имеют ли все экземпляры резервных копий по крайней мере одну успешную архивацию каждый день. Можно просмотреть соблюдение политик по периоду времени или экземпляру резервной копии.
...
После создания приложения логики необходимо авторизовать подключения к журналам Azure Monitor и Office 365. Для этого перейдите к **Logic Apps** в портал Azure и найдите имя созданной задачи. При выборе пункта меню **подключения API** открывается список подключений API, которые необходимо авторизовать.

###### <a name="customize-azure-backup-reports"></a>Настройка отчетов Azure Backup

Отчеты о резервном копировании используют функции в журналах Azure Monitor. Эти функции работают с данными в необработанных Azure Backup таблицах в LA и возвращают форматированные данные, которые позволяют легко получать сведения обо всех сущностях, связанных с резервным копированием, с помощью простых запросов.
