...
1. Задайте для "queryType" значение "семантика", а для "Куерилангуаже" — "en-US". Оба параметра являются обязательными.
...
1. Необязательно, но рекомендуется, задать "searchFields".
...
1. Удалите предложения orderBy, если они существуют в существующем запросе. Семантическая Оценка используется для упорядочивания результатов и при включении явной логики сортировки возвращается ошибка HTTP 400.

1. При необходимости добавьте "Answers" в значение "Extract" и укажите количество ответов, если требуется более 1.
